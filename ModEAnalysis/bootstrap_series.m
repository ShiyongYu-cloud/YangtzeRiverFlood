function Xb = bootstrap_series(x,sim,L,nsamples)
%function for bootstrapping a time series
x = x(:); 
x_mean = movmean(x,31);
z = x - x_mean;
Xb = zeros(size(x,1),nsamples);
for i = 1:nsamples
    zb = stationaryBB(z,sim,L);% bootstrap the stationary series
    Xb(:,i) = zb + x_mean; %adding the mean back
end    
return;
%%
